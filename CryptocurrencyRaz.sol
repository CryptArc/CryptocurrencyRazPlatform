...
contract CryptocurrencyRaz is Ownable {
    ...
    function CryptocurrencyRaz() public 
    {
        owner = ownerWallet;
        Setup();
    }
    ...
    function getAvailableNumbersForRaz (uint razNumber) public constant returns (uint[])
    {
        require(razNumber>=1 && razNumber<=numberOfRazzes);
        razList = empty;
        for (uint i=1;i<=maxBetsForEachRaz[razNumber];i++)
    ...
    function declareWinnerForRaz(uint razNumber,uint winningNumber) public onlyOwner
    {
        require(razNumber>=1 && razNumber<=numberOfRazzes);
        ...
        ownerWallet.transfer(ownerPrizeMoneyForEachRaz[razNumber]);
        resetRaz(razNumber);
    }
    
    function UserBetsInRaz(address userAddress, uint razNumber) public constant returns (uint[])
    {
        require(razNumber>=1 && razNumber<=numberOfRazzes);
        razList = empty;
        for (uint i=1;i<=maxBetsForEachRaz[razNumber];i++)
